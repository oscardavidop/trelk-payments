...
export class PaypalController {
   ...
   */
  @Get('subscribe')
  async subscribe(@Query('tg_id') telegramId: string, @Res() res: Response) {
    if (!telegramId) {
    ...
    try {
      ...
      const user = await this.subscriptionService.getOrCreateUser(parsedTgId);

      // Renderizar página HTML con SDK de PayPal
      const html = ``.replace('TU_CLIENT_ID', paypalClientId)

      res.setHeader('Content-Type', 'text/html; charset=utf-8');
      res.sendFile("/home/trelkbot17/tg-paypal-bot/public/plans.html");
    } catch (error: any) {
...
<body>
  ...
  async cancel(@Body() body: { tg_id: number; subscription_id: string }) {
    ...
    try {
      ...
      if (subscription.user.telegramId !== body.tg_id) {
